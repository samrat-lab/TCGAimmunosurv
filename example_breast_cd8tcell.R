...
#oncoplots
oncoplot(maf = cancer_maf, fontSize = 1, titleFontSize = 1.5, legendFontSize = 1.5, top = 2)

#Transition and Transversion plot
titv = titv(maf = cancer_maf, plot = TRUE)

#Lollipop Plot of the gene of interest
lollipopPlot(maf = cancer_maf, gene = "TP53", AACol = "HGVSp_Short", showMutationRate = TRUE)

# somatic interactions
somaticInteractions(maf = cancer_maf, top = 2, pvalue = c(0.05, 0.1))
...
#combined effect of the genes
plot_km_combined(strata_data, clinical_data_filtered, significant_genes = significant_genes_all)
...
cds <- perform_pseudotime(cd8_tcells, num_dim = 20, root_cluster = "Naive CD8+ T")

plot_cd8_trajectory(cds, cd8_tcells, label_size = 4)

graph_test_res <- run_graph_test(cds, neighbor_graph = "principal_graph", cores = 11)

genes <- get_top_combined_genes(graph_test_res, q_value_threshold = 0.05)
